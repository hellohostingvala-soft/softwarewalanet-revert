-- V136__create_orders_licenses.sql
-- Mirrors supabase/migrations/20260301013122_create_orders_licenses.sql

-- Helper function: reusable admin check to avoid repeated subquery in RLS policies
CREATE OR REPLACE FUNCTION public.is_admin_or_boss(uid UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles
    WHERE user_id = uid AND role IN ('master', 'super_admin', 'boss_owner')
  );
$$;
...
FOR SELECT USING (user_id = auth.uid() OR public.is_admin_or_boss(auth.uid()));

CREATE POLICY "System can insert orders" ON public.orders
FOR INSERT WITH CHECK (true);

CREATE POLICY "Admins can update orders" ON public.orders
FOR UPDATE USING (public.is_admin_or_boss(auth.uid()));

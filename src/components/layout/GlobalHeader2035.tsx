...
const GlobalHeader2035 = ({ roleName, maskedId, lowDataMode, onChatToggle }: HeaderProps) => {
  ...
  const handleInfluencer = useCallback(() => goTo('/influencer'), [goTo]);
  const handleJoinDeveloper = useCallback(() => goTo('/developer/join'), [goTo]);
  const handleApplyJob = useCallback(() => goTo('/jobs/apply'), [goTo]);
  const handleLogin = useCallback(() => goTo('/auth'), [goTo]);
  const handleBossPortal = useCallback(() => {
    if (userRole === 'boss_owner' || userRole === 'ceo' || userRole === 'master') {
      goTo('/super-admin');
    } else {
      goTo('/auth?redirect=/super-admin');
    }
  }, [goTo, userRole]);
        ...
        <div className="flex items-center gap-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-80">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem className="justify-center text-primary" onClick={() => goTo('/notifications')}>
